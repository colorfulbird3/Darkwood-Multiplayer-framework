...
internal sealed class SaveTransferRuntime : MonoBehaviour
{
	...
	private void OnDestroy()
	{
		...
		try
		{
			if ((Object)(object)Singleton<SaveManager>.Instance != (Object)null)
			{
				SaveManager instance = Singleton<SaveManager>.Instance;
				instance.onFinishedLoading = (saveDelegate)Delegate.Remove((Delegate)(object)instance.onFinishedLoading, (Delegate)new saveDelegate(OnDownloadedSaveFinished));
			}
		}
		catch
		{
	...
	private void Update()
	{
		...
		if (!Core.mainMenu && (Object)(object)Player.Instance != (Object)null && Time.unscaledTime >= nextStabilityProbe)
		{
	...
	private void CompleteIncoming()
	{
		try
		{
			...
			((MonoBehaviour)this).StartCoroutine(LoadDownloadedSave());
		}
		catch (Exception ex)
		{
	...
	private IEnumerator LoadDownloadedSave()
	{
		...
		SaveManager instance = Singleton<SaveManager>.Instance;
		SaveState val = instance.loadGameProfiles();
		if (val == null || val.profiles == null)
		{
			throw new InvalidDataException("Downloaded profiles file could not be loaded.");
		}
		GameProfile val2 = val.profiles.FirstOrDefault((GameProfile p) => p != null && p.id == incomingProfileId);
		if (val2 == null || !val2.Active)
		{
		...
		instance.onFinishedLoading = (saveDelegate)Delegate.Remove((Delegate)(object)instance.onFinishedLoading, (Delegate)new saveDelegate(OnDownloadedSaveFinished));
		instance.onFinishedLoading = (saveDelegate)Delegate.Combine((Delegate)(object)instance.onFinishedLoading, (Delegate)new saveDelegate(OnDownloadedSaveFinished));
		Plugin.Log.LogInfo((object)$"Loading downloaded multiplayer profile {val2.id}: day {val2.day}, chapter {val2.chapter}.");
		UI instance2 = Singleton<UI>.Instance;
		((MonoBehaviour)instance2).StartCoroutine(instance2.initLoadGame());
	}
	...
	private static int ExtractBundle(byte[] compressed, string destination)
	{
		...
		string text = Path.GetFullPath(destination).TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar;
		long num3 = 0L;
		for (int i = 0; i < num2; i++)
		{
			string text2 = binaryReader.ReadString().Replace('/', Path.DirectorySeparatorChar);
			if (!IsAllowedRelativePath(text2, num))
			{
				throw new InvalidDataException("Rejected save path: " + text2);
			}
			...
			{
				throw new EndOfStreamException(text2);
			}
	...
	private static bool IsAllowedRelativePath(string relative, int profileId)
	{
		...
		string text = "prof" + profileId + Path.DirectorySeparatorChar;
		if (!relative.StartsWith(text, StringComparison.OrdinalIgnoreCase))
		{
			return false;
		}
		string name = relative.Substring(text.Length);
	...
	private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
	{
		...
		{
			Plugin.Log.LogInfo((object)("Downloaded save scene loaded: " + ((Scene)(ref scene)).name));
		}
	}
}
