...
public sealed class Plugin : BaseUnityPlugin
{
	...
	{
		GameObject gameObject = new GameObject("DarkwoodMultiplayerNetworkV2");
		UnityEngine.Object.DontDestroyOnLoad(gameObject);
		transport = gameObject.AddComponent<LegacyTelepathyTransport>();
		...
		gameObject.AddComponent<MirrorRuntimePump>();
		gameObject.AddComponent<SaveTransferRuntime>();
		gameObject.AddComponent<SyncRuntime>();
		gameObject.AddComponent<WorldStateSync>();
		gameObject.AddComponent<NetworkStatsRuntime>();
	...
	private void OnGUI()
	{
		if (show)
		{
			window = GUILayout.Window(((UnityEngine.Object)this).GetInstanceID(), window, Draw, "Darkwood 联机框架设置");
		}
	}

	private void Draw(int id)
	{
		GUILayout.Label("联机人数（新随机战利品按此倍率生成）");
		GUILayout.BeginHorizontal();
		for (int i = 1; i <= 8; i++)
		{
			if (GUILayout.Button((i == Players.Value) ? $"[{i}]" : i.ToString(), GUILayout.Width(43f)))
			{
				SetPlayers(i);
			}
		}
		GUILayout.EndHorizontal();
		bool flag = GUILayout.Toggle(ScaleLoot.Value, "按人数缩放随机容器战利品");
		if (flag != ScaleLoot.Value)
		{
			ScaleLoot.Value = flag;
			((BaseUnityPlugin)this).Config.Save();
		}
		bool flag2 = GUILayout.Toggle(HostSceneAuthority.Value, "客户端跟随主机场景");
		if (flag2 != HostSceneAuthority.Value)
		{
			HostSceneAuthority.Value = flag2;
			((BaseUnityPlugin)this).Config.Save();
		}
		bool flag3 = GUILayout.Toggle(ShowNetworkStats.Value, "显示右上角 FPS/延迟/丢包");
		...
		address = GUILayout.TextField(address ?? "127.0.0.1");
		GUILayout.Label("端口");
		port = GUILayout.TextField(port ?? "7777");
		GUILayout.BeginHorizontal();
		GUI.enabled = !NetworkClient.active && !NetworkServer.active;
		if (GUILayout.Button("启动主机 (F1)"))
		{
			StartHost();
		}
		if (GUILayout.Button("连接主机 (F2)"))
		{
			StartClient();
		}
		GUI.enabled = NetworkClient.active || NetworkServer.active;
		if (GUILayout.Button("停止 (F3)"))
		{
			Stop();
		}
		GUI.enabled = true;
		...
		{
			GUILayout.Label("存档：" + SaveTransferRuntime.StatusText);
		}
		GUILayout.Label("客户端应停留在主菜单连接；连接后会自动下载并加载主机当前存档。");
		GUILayout.Label("下载存档保存在 BepInEx/DarkwoodMPClientSaves，不覆盖本地槽位。");
		GUILayout.Label("F6 关闭");
	...
	{
		if (manager == null || NetworkClient.active || NetworkServer.active || !Apply())
		{
			return;
		}
		try
	...
	private IEnumerator StartHostProfile(int profileId)
	{
		...
		while ((Singleton<SaveManager>.Instance == null || Singleton<UI>.Instance == null) && Time.realtimeSinceStartup < deadline)
		{
			yield return null;
		}
		if (Singleton<SaveManager>.Instance == null || Singleton<UI>.Instance == null)
		{
			Log.LogError((object)"Profile test launch timed out waiting for game singletons.");
			yield break;
		}
		MainMenu.SaveState saveState = Singleton<SaveManager>.Instance.loadGameProfiles();
		GameProfile gameProfile = ((saveState != null && saveState.profiles != null) ? saveState.profiles.FirstOrDefault((GameProfile p) => p != null && p.id == profileId && p.Active) : null);
		if (gameProfile == null)
		{
		...
		instance.StartCoroutine(instance.initLoadGame());
		while ((Core.mainMenu || Player.Instance == null) && Time.realtimeSinceStartup < deadline)
		{
			yield return null;
		}
