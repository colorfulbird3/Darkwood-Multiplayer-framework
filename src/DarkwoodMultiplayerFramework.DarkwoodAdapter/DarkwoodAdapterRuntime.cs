...
public sealed class DarkwoodAdapterRuntime : MonoBehaviour
{
    ...
    public void StopNetwork()
    {
        ...
        outgoing.Clear(); readyPeers.Clear(); sentSaves.Clear(); sentSnapshots.Clear(); incomingSave=null; incomingSnapshot=null; replication.RestoreSimulation(); remotePlayers.Clear(); ActiveClientSaveDirectory=string.Empty; sessionError=string.Empty;
    ...
    private void OnPeerDisconnected(int connectionId){outgoing.Remove(connectionId);readyPeers.Remove(connectionId);sentSaves.Remove(connectionId);sentSnapshots.Remove(connectionId);remotePlayers.Remove(connectionId);}
    ...
    private void OnHostMessage(int peer,ProtocolEnvelope envelope)
    {
        try
        {
            ...
            else if(envelope.MessageType==ProtocolMessageType.WorldSnapshotApplied){var applied=ReplicationProtocolCodec.DecodeWorldSnapshotApplied(envelope.Payload);if(!sentSnapshots.TryGetValue(peer,out var expected)||expected!=applied.SnapshotId||applied.Scene!=CurrentScene||applied.RegistryDigest!=RegistryDigest)throw new InvalidDataException("Snapshot acknowledgement does not match active snapshot.");readyPeers.Add(peer);Queue(peer,ProtocolMessageType.Ready,ReplicationProtocolCodec.Encode(new ReadyMessage(CurrentScene,RegistryDigest)));SendHostPose(peer);log?.LogInfo($"Peer {peer} READY after applying snapshot {applied.SnapshotId}, {applied.EntityCount} entities.");}
            else if(envelope.MessageType==ProtocolMessageType.PlayerPose){var pose=ReplicationProtocolCodec.DecodePlayerPose(envelope.Payload);if(!readyPeers.Contains(peer)||pose.Scene!=CurrentScene)return;pose=new PlayerPoseMessage(peer,pose.Sequence,CurrentScene,pose.X,pose.Y,pose.Z,pose.Qx,pose.Qy,pose.Qz,pose.Qw,pose.Flags,pose.TorsoClip,pose.TorsoFrame,pose.LegsClip,pose.LegsFrame);remotePlayers.Apply(pose,0);var payload=ReplicationProtocolCodec.Encode(pose);foreach(var readyPeer in readyPeers.ToArray())if(readyPeer!=peer)Queue(readyPeer,ProtocolMessageType.PlayerPose,payload);}
        }
