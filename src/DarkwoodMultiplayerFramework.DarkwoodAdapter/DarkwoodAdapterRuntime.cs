...
public sealed class DarkwoodAdapterRuntime : MonoBehaviour
{
    ...
    private void OnHostMessage(int peer,ProtocolEnvelope envelope)
    {
        try
        {
            ...
            else if(envelope.MessageType==ProtocolMessageType.InventoryState){var inventory=ReplicationProtocolCodec.DecodeInventoryState(envelope.Payload);var id=new EntityId(inventory.Value,inventory.Persistent);if(replication.TryGetInventoryState(id,out var current)&&!ContainerRevisionGate.TryAdvance(inventory.Revision,current.Revision,out _)){log?.LogWarning($"容器并发冲突：ID={inventory.Value:X16}，玩家 {peer} 基于版本 {inventory.Revision}，主机当前 {current.Revision}——拒绝该次上报并回权威状态。");Queue(peer,ProtocolMessageType.InventoryState,ReplicationProtocolCodec.Encode(current));}else if(!replication.Apply(inventory)){missingEntities.Add(new EntityId(inventory.Value,inventory.Persistent));log?.LogWarning($"忽略缺失实体的容器状态：ID={inventory.Value:X16}，名称={inventory.Name}（主机运行时生成物，等待 Spawn 生命周期补发）。");}else{foreach(var readyPeer in readyPeers.ToArray())Queue(readyPeer,ProtocolMessageType.InventoryState,envelope.Payload);log?.LogInfo($"主机已应用客户端容器状态并转发：ID={inventory.Value:X16}，玩家 {peer}，版本 {inventory.Revision}，槽位 {inventory.Slots.Length}。");}}
    ...
    private void OnClientMessage(ProtocolEnvelope envelope)
    {
        try
        {
            ...
            else if(envelope.MessageType==ProtocolMessageType.InventoryState){var inventory=ReplicationProtocolCodec.DecodeInventoryState(envelope.Payload);var id=new EntityId(inventory.Value,inventory.Persistent);if(replication.TryGetInventoryState(id,out var local)){if(inventory.Revision<local.Revision){var taken=DarkwoodContainerTakePatch.DrainPendingTakes(id);if(taken.Count>0){foreach(var item in taken)DarkwoodInventoryAdapter.RemoveFromPlayerInventory(item.Type,item.Amount);log?.LogWarning($"容器并发冲突补偿：ID={inventory.Value:X16}，本次拿取未生效，已从背包退回 {taken.Count} 类物品，容器回滚为权威版本 {inventory.Revision}。");}}else DarkwoodContainerTakePatch.ClearPendingTakes(id);}if(!replication.Apply(inventory)){missingEntities.Add(new EntityId(inventory.Value,inventory.Persistent));log?.LogWarning($"忽略缺失实体的容器状态：ID={inventory.Value:X16}，名称={inventory.Name}（主机运行时生成物，等待 Spawn 生命周期补发）。");}}
