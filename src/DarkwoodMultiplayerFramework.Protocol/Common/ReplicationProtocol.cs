...
public static class ProtocolVersions
{
    ...
    public const string Framework = "0.8.9.6-pre.1";
}
...
public static class ReplicationProtocolCodec
{
    ...
    // P0-D/E：容器 grab→HeldItem / HeldToInventory payload
    public static byte[] Encode(ContainerGrabPayload m)=>Write(w=>{w.Write(m.SlotIndex);w.Write(m.Amount);});
    public static ContainerGrabPayload DecodeContainerGrab(byte[] p)=>Read(p,r=>new ContainerGrabPayload(r.ReadInt32(),r.ReadInt32()));
    public static byte[] Encode(HeldItemStatePayload m)=>Write(w=>{WriteString(w,m.Type);w.Write(m.Amount);w.Write(m.Durability);w.Write(m.Quality);w.Write(m.Recipe);w.Write(m.Ammo);});
    public static HeldItemStatePayload DecodeHeldItemState(byte[] p)=>Read(p,r=>new HeldItemStatePayload(ReadString(r),r.ReadInt32(),r.ReadSingle(),r.ReadInt32(),r.ReadBoolean(),r.ReadInt32()));
    ...
    // v0.9.2 P0-4/P0-8：ContainerCommit / DropCommit / PickupCommit / DropCommitAck——原子事务版本号
    public static byte[] Encode(ContainerCommitMessage m)=>Write(w=>{WriteGuid(w,m.TransactionId);w.Write(m.ContainerValue);w.Write(m.ContainerPersistent);w.Write(m.BaseContainerRevision);WriteInventorySlots(w,m.ContainerSlots);w.Write(m.PlayerId);w.Write(m.PlayerInventoryRevision);WriteInventorySlots(w,m.BackpackAfter);WriteInventorySlots(w,m.HotbarAfter);});
    public static ContainerCommitMessage DecodeContainerCommit(byte[] p)=>Read(p,r=>{var tid=ReadGuid(r);var cv=r.ReadUInt64();var cp=r.ReadBoolean();var baseRev=r.ReadInt32();var cslots=ReadInventorySlots(r);var pid=r.ReadInt32();var pir=r.ReadInt32();var bp=ReadInventorySlots(r);var hb=ReadInventorySlots(r);return new ContainerCommitMessage(tid,cv,cp,baseRev,cslots,pid,pir,bp,hb);});
    public static byte[] Encode(PickupCommitMessage m)=>Write(w=>{WriteGuid(w,m.TransactionId);w.Write(m.RuntimeEntityId);w.Write(m.Persistent);WriteString(w,m.ItemType);w.Write(m.Amount);w.Write(m.PlayerId);w.Write(m.PlayerInventoryRevision);WriteInventorySlots(w,m.BackpackAfter);WriteInventorySlots(w,m.HotbarAfter);});
    public static PickupCommitMessage DecodePickupCommit(byte[] p)=>Read(p,r=>{var tid=ReadGuid(r);var rid=r.ReadUInt64();var pst=r.ReadBoolean();var t=ReadString(r);var amt=r.ReadInt32();var pid=r.ReadInt32();var pir=r.ReadInt32();var bp=ReadInventorySlots(r);var hb=ReadInventorySlots(r);return new PickupCommitMessage(tid,rid,pst,t,amt,pid,pir,bp,hb);});
    public static byte[] Encode(DropCommitMessage m)=>Write(w=>{WriteGuid(w,m.TransactionId);w.Write(m.LocalDropToken);WriteString(w,m.ItemType);w.Write(m.Amount);w.Write(m.Durability);w.Write(m.Quality);w.Write(m.Recipe);w.Write(m.X);w.Write(m.Y);w.Write(m.Z);w.Write(m.Qx);w.Write(m.Qy);w.Write(m.Qz);w.Write(m.Qw);w.Write(m.PlayerId);w.Write(m.PlayerInventoryRevision);WriteInventorySlots(w,m.BackpackAfter);WriteInventorySlots(w,m.HotbarAfter);});
    ...
    private static void WriteGuid(BinaryWriter w,Guid g){w.Write(g.ToByteArray());}
    private static Guid ReadGuid(BinaryReader r)=>new Guid(ReadExact(r,16));
